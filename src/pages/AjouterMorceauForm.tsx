...
const AjouterMorceauForm: React.FC<AjouterMorceauFormProps> = ({ onClose, onMorceauAdded, ensembleId }) => {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            let errorMessage = "Erreur lors de l'ajout du morceau. Veuillez réessayer.";
        ...
        <div className="modal-backdrop">
            <div className="modal-content">
                ...
                <form onSubmit={handleSubmit}>
                    ...
                    <label className="form-label" htmlFor="compositeur">Compositeur</label>
                    <input
                        className="form-input"
                        id="compositeur"
                        type="text"
                    ...
                    {message && <p className={`form-message ${message.includes('succès') ? 'success' : 'error'}`}>{message}</p>}

                    <button 
                        type="submit" 
                        className="submit-button" 
                        // Désactivation si en cours de soumission ou si un champ requis est vide
                        disabled={isSubmitting || !titre || !compositeur || !genre || !ensembleId}
                    >
                        <FaSave /> {isSubmitting ? 'Enregistrement...' : 'Ajouter le Morceau'}
                    </button>
                </form>
            </div>
        </div>
